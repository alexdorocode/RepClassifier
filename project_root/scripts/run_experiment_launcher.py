...
def main(cfg: DictConfig):
    ...
    handler.set_sweeps_config_phase_1()
    ...
    final_classifier_configs = handler.get_classifier_ready_configs()
    ...
    for cfg_dict in final_classifier_configs:
        cfg_custom = OmegaConf.create(cfg_dict)
        cfg_custom = OmegaConf.merge(cfg, cfg_custom)
        print(f"🔧 Running with config: {OmegaConf.to_yaml(cfg_custom)}")
        launcher = ClassifierLauncher(cfg_custom)
        launcher.run()
