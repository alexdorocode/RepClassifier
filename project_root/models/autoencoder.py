...
class Autoencoder(nn.Module):
    ...
    def load_from_checkpoint(path, input_dim, latent_dim, activation="relu", device="cpu"):
        """
        Load an Autoencoder model from a .pt checkpoint.

        Args:
            path (str): Path to the saved state_dict (.pt file)
            input_dim (int): Input dimension of the autoencoder
            latent_dim (int): Latent dimension (output of encoder)
            activation (str): Activation function used during training
            device (str): Device to load the model onto ("cpu" or "cuda")

        Returns:
            Autoencoder: Model loaded with weights and set to eval mode
        """
        model = Autoencoder(input_dim=input_dim, latent_dim=latent_dim, activation_name=activation)
